...
{
private:
	void operator = (const pxtnWoice& src){ (void)src; }
	pxtnWoice       (const pxtnWoice& src) : pxtnData() { (void)src; }
...
public :
	...
	pxtnERR Tone_Ready_sample  ( const pxtnPulse_NoiseBuilder *ptn_bldr, int32_t ch, int32_t sps );
	pxtnERR Tone_Ready_envelope( int32_t sps );
	pxtnERR Tone_Ready         ( const pxtnPulse_NoiseBuilder *ptn_bldr, int32_t ch, int32_t sps );
};
